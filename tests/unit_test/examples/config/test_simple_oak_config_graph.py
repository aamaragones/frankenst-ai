...
def _patch_simple_oak_runtime(
    published_runtime: Callable[..., LLMRuntime], response_content: str = "oak-response"
) -> BaseChatModel:
    fake_model = cast(
        BaseChatModel, ToolBindingFakeModel(response_content=response_content)
    )
    published_runtime(model=fake_model)
    return fake_model
...
def test_simple_oak_config_graph_build_runtime_resolves_all_runtime_dependencies(
    ...
    fake_model = _patch_simple_oak_runtime(published_runtime)
...
def test_simple_oak_config_graph_declares_nodes_edges_and_runnable_builders(
    ...
    _patch_simple_oak_runtime(published_runtime)
    layout = simple_oak_module.SimpleOakConfigGraph()
...
def test_simple_oak_config_graph_compiles_and_runs_with_workflow_builder(
    ...
    _patch_simple_oak_runtime(published_runtime, response_content="oak-layout-response")
    ...
    result = asyncio.run(
        compiled.ainvoke({"messages": [HumanMessage(content="Hi Oak")]})
    )
