...
import utils.rag.local_chroma as local_chroma_module
...
def test_resolve_configured_path_uses_base_dir_for_relative_paths(
    tmp_path: Path,
) -> None:
...
def test_resolve_configured_path_preserves_absolute_paths(tmp_path: Path) -> None:
    ...
    assert (
        common_module.resolve_configured_path(absolute_path, base_dir) == absolute_path
    )
...
def test_local_chroma_uses_default_directories_without_constants_or_cwd(
    ...
    monkeypatch.chdir(other_dir)
    monkeypatch.setattr(
        local_chroma_module, "get_project_root_path", lambda: project_root
    )
    monkeypatch.setattr(
        local_chroma_module,
        "get_default_artifacts_directory",
    ...
    assert (
        local_chroma_module.get_default_local_docstore_directory()
        == project_root / "artifacts" / ".docstore"
    )
    assert (
        local_chroma_module.get_default_local_rag_docs_directory()
        == project_root / "artifacts" / "rag_docs"
    )
    assert local_chroma_module._resolve_local_storage_path(
        None, local_chroma_module.get_default_local_chroma_directory
    ) == (project_root / "artifacts" / ".chromadb")
...
def test_astream_updates_accepts_command_input() -> None:
    ...
    result = asyncio.run(common_module.astream_updates(cast(Any, graph), command_input))
