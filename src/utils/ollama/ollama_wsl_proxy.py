"""Resolve the Ollama base URL, proxying WSL requests to the Windows host when needed."""
...
def _ensure_wsl_ollama_proxy(
    proxy_host: str, proxy_port: int, target_base_url: str
) -> str | None:
...
def _stream(pipe: IO[bytes] | None) -> IO[bytes]:
    if pipe is None:
        raise RuntimeError("curl.exe was started without the expected pipe")
    return pipe


class OllamaWSLProxyHandler(BaseHTTPRequestHandler):
    """Forward each request to the Windows-side Ollama through `curl.exe`."""
    ...
    def _proxy_request(self) -> None:
        ...
        while b"\r\n\r\n" not in raw_response:
            chunk = _stream(process.stdout).read(1)
            if not chunk:
                break
        ...
        if not separator:
            stderr_output = (
                _stream(process.stderr).read().decode("utf-8", errors="replace")
            )
            self.send_error(
                502, stderr_output or "Invalid response from Windows Ollama relay"
            )
            process.kill()
            return
        ...
        while True:
            chunk = _stream(process.stdout).read(8192)
            if not chunk:
                break
            try:
