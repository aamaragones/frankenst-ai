"""Azure Blob Storage helpers for the Event Grid indexing example."""
...
def upload_file_to_blob(
    blob_path: str, content: str, container_name: str, overwrite: bool = True
) -> str:
    blob_service_client = get_blob_service_client()
...
def download_pdf_from_blob(blob_path: str, container_name: str) -> str:
    ...
    return download_blob_to_temp_file(
        blob_path=blob_path, container_name=container_name
    )


def parse_blob_subject(subject: str) -> tuple[str, str]:
    """Split `/blobServices/default/containers/{container}/blobs/{path}` into (path, container)."""
    ...
    try:
        ...
        raise ValueError(
            "Invalid subject format: missing 'containers' or 'blobs' segment"
        ) from None
