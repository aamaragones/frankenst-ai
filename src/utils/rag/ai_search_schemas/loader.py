"""Load the registered Azure AI Search index definitions shipped as JSON."""
...
def load_ai_search_index_definition(schema_name: str, index_name: str) -> SearchIndex:
    ...
    if not isinstance(schema, dict):
        raise ValueError(
            f"The index schema '{schema_path}' must contain a JSON object at the root."
        )
    ...
    if index_definition is None:
        raise ValueError(
            f"The index schema '{schema_path}' could not be deserialized into a SearchIndex."
        )
