"""Build or reload the local Chroma multi-vector store used by the RAG layouts."""
...
from utils.rag.langchain_unstructured_indexer import (
...
def get_local_retriever_storage(
    ...
    persist_directory: str | Path | None = None,
    docstore_directory: str | Path | None = None,
) -> tuple[Chroma, BaseStore[str, Document]]:
...
def _get_pdf_paths(rag_docs_directory: str | Path | None) -> list[Path]:
    ...
    if not pdf_paths:
        raise ValueError(
            f"No PDF documents found in {directory} to bootstrap the local Chroma collection."
        )
