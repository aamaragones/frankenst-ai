"""Path, resource and graph-streaming helpers shared by the examples."""
...
from utils.config_loader import read_text_resource
...
def load_and_clean_text_file(
    file_path: str | Path | Traversable, remove_empty_lines: bool = False
) -> str:
    try:
...
def save_text_to_artifact(
    ...
    target_dir = resolve_configured_path(
        ...
        else get_default_artifacts_directory(),
        project_root_path,
    )
...
async def astream_updates(
    ...
    """Log every `updates` event and return the last one; a `Command` resumes a run."""
    events: list[Any] = []
    async for event in graph.astream(
        message_input, runnable_config, stream_mode="updates"
    ):
        events.append(event)
        logger.info("%s", event)
