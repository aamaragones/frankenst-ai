"""YAML readers: `$(a.b)` interpolation, and a validated node registry."""
...
def read_text_resource(
    resource_path: str | Path | Traversable, encoding: str = "utf-8"
) -> str:
...
def read_yaml(path_to_yaml: str | Path | Traversable) -> dict[str, Any]:
    try:
        yaml_content = yaml.safe_load(read_text_resource(path_to_yaml))
        ...
        if not parsed_yaml_context:
            raise ValueError(
                f"The configuration file '{path_to_yaml}' must not be an empty mapping."
            )

        return parsed_yaml_context
    except FileNotFoundError:
        raise FileNotFoundError(
            f"The configuration file '{path_to_yaml}' does not exist."
        ) from None
        ...
        raise RuntimeError(
            f"Unable to read configuration file '{path_to_yaml}'"
        ) from exc
...
def load_node_registry(
    ...
    try:
        data = yaml.safe_load(read_text_resource(path_to_yaml))
        if data is None:
            raise ValueError(f"The configuration file '{path_to_yaml}' is empty.")

        if not isinstance(data, dict):
            raise ValueError(
                f"The node registry '{path_to_yaml}' must contain a YAML mapping at the root."
            )

        nodes = data.get("nodes")
        if not isinstance(nodes, list) or not nodes:
            raise ValueError(
                f"The node registry '{path_to_yaml}' must define a non-empty top-level 'nodes' list."
            )
        ...
        for entry in nodes:
            if not isinstance(entry, dict):
                raise ValueError(
                    f"The node registry '{path_to_yaml}' must contain only mapping entries inside 'nodes'."
                )
