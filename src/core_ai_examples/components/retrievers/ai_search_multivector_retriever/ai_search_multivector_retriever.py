...
class AISearchMultiVectorRetriever:
    ...
    def _search(
        self, query: str, k: int = 5, embed: bool = True
    ) -> list[dict[str, Any]]:
    ...
    def _parse_results(
        self, results: list[dict[str, Any]], metadata_as_content: bool = True
    ) -> dict[str, list[str]]:
        """Groups documents by type, optionally appending metadata to content."""
        grouped: dict[str, list[str]] = {"texts": [], "images": []}
        for doc in results:
            t = doc.get("type")
            c = doc.get("content", "")
    ...
    def get_context(
        ...
        return {"texts": context_text.strip(), "images": context_images}
